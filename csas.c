...
ZEND_BEGIN_ARG_INFO_EX(sanitizer_arg_info, 0, 0, 1)
    ZEND_ARG_INFO(0, string)
ZEND_END_ARG_INFO()
...
zend_function_entry csas_functions[] = {
    ...
    PHP_FE(html_quoted_sanitize, sanitizer_arg_info)
    PHP_FE(html_unquoted_sanitize, sanitizer_arg_info)
    PHP_FE(js_sanitize, sanitizer_arg_info)
    PHP_FE(url_start_sanitize, sanitizer_arg_info)
    PHP_FE(url_general_sanitize, sanitizer_arg_info)
    PHP_FE(url_query_sanitize, sanitizer_arg_info)
    {NULL, NULL, NULL}
};
/* }}} */
...
static struct csas_overridden_fucs /* {{{ */ {
    ...
    // pdo_statement class methods
    php_func pdo_fetch;
    php_func pdo_fetch_all;
...
static char *sanitize_for_context(char *s, int safety, int *len) {
    ...
    switch (get_safety_needed()) {
            ...
            case PHP_CSAS_SAFE_URL_START:
                return url_start_escape_sanitize(s, len);
            case PHP_CSAS_SAFE_URL_QUERY:
...
static void php_csas_override_functions(TSRMLS_D) /* {{{ */ {
    ...
    php_csas_override_class_func(c_pdo_statement, sizeof(c_pdo_statement), f_pdo_fetch, sizeof(f_pdo_fetch),
                                 PHP_FN(csas_pdo_fetch), &CSAS_O_FUNC(pdo_fetch) TSRMLS_CC);
    php_csas_override_class_func(c_pdo_statement, sizeof(c_pdo_statement), f_pdo_fetch_all, sizeof(f_pdo_fetch_all),
                                 PHP_FN(csas_pdo_fetch_all), &CSAS_O_FUNC(pdo_fetch_all) TSRMLS_CC);
    php_csas_override_class_func(c_pdo_statement, sizeof(c_pdo_statement), f_pdo_fetch_column, sizeof(f_pdo_fetch_column),
                                 PHP_FN(csas_pdo_fetch_column), &CSAS_O_FUNC(pdo_fetch_column) TSRMLS_CC);
...
PHP_FUNCTION(csas_pdo_fetch) {
    CSAS_O_FUNC(pdo_fetch)(INTERNAL_FUNCTION_PARAM_PASSTHRU);
...
PHP_FUNCTION(csas_pdo_fetch_all) {
    CSAS_O_FUNC(pdo_fetch_all)(INTERNAL_FUNCTION_PARAM_PASSTHRU);
...
PHP_FUNCTION(csas_pdo_fetch_column) {
    CSAS_O_FUNC(pdo_fetch_column)(INTERNAL_FUNCTION_PARAM_PASSTHRU);
...
/**********************************************************
Sanitizer Wrapper functions:
    These functions allow access to the sanitizers via PHP
**********************************************************/
...
PHP_FUNCTION(html_quoted_sanitize){
    ...
    str = html_escape_sanitize(str, &str_len);
...
PHP_FUNCTION(html_unquoted_sanitize){
    ...
    str = html_unquoted_escape_sanitize(str, &str_len);
    RETURN_STRINGL(str, str_len, 0);
}
...
PHP_FUNCTION(js_sanitize){
    ...
    str = javascript_escape_sanitize(str, &str_len);
    RETURN_STRINGL(str, str_len, 0);
}
...
PHP_FUNCTION(url_query_sanitize){
    ...
    str =  url_query_escape_sanitize(str, &str_len);
    RETURN_STRINGL(str, str_len, 0);
}
...
PHP_FUNCTION(url_start_sanitize){
    ...
    str = url_start_escape_sanitize(str, &str_len);
    RETURN_STRING(str, 0);
}
PHP_FUNCTION(url_general_sanitize){
    ...
    str =  url_general_escape_sanitize(str, &str_len);
    RETURN_STRINGL(str, str_len, 0);
}
